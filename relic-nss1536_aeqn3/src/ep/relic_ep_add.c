...
//==================FOR G2================	//to be edtied and checked below 0617
// y^2 = x^3 - 3/4 x
static void ep_G2_add_projc_imp(ep_t r, const ep_t p, const ep_t q) {
	...
	RLC_TRY {					
		...
		fp_sub(t3, t3, t4);	
			...
			fp_dbl(r->z, t4); fp_add(r->z, r->z, t4); fp_neg(r->z, r->z); fp_hlv(r->z, r->z); fp_hlv(r->z, r->z);
			// fp_dbl(r->x, t2);	
			// fp_add(r->x, r->x, t2);		
			...
			fp_dbl(t4, t2); fp_add(t2, t4, t2); fp_neg(t2, t2); fp_hlv(t2, t2); fp_hlv(t2, t2);
			fp_add(t1, t1, t2);
			fp_sub(t2, t0, t2);
			// ep_curve_mul_a(t2, t2);	
			fp_dbl(t4, t2); fp_add(t2, t4, t2); fp_neg(t2, t2); fp_hlv(t2, t2); fp_hlv(t2, t2);
