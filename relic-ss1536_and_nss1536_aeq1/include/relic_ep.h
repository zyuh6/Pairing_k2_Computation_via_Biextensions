...
enum {
    ...
    EP_SG54,
	/** Non-supersingular curves with embedding degree 2. */
    EP_NSS2,
};
...
enum {
	...
	K1_P3072,
	/** 1536-bit non-supersingular curve. */
	NSS_P1536,
	/** twist of 1536-bit non-supersingular curve. */
	NSS_P1536_G2,
};
...
//==================FOR G2================
#define ep_G2_add(R, P, Q)		ep_G2_add_projc(R, P, Q)
#define ep_G2_dbl(R, P)		ep_G2_dbl_projc(R, P)
#define ep_G2_mul(R, P, K)		ep_G2_mul_lwnaf(R, P, K)

void ep_G2_add_projc(ep_t r, const ep_t p, const ep_t q);
void ep_G2_dbl_projc(ep_t r, const ep_t p);
void ep_G2_mul_lwnaf(ep_t r, const ep_t p, const bn_t k);
...
#if !defined(EP_SUPER) && FP_PRIME == 1536
void ep_G2_rhs(fp_t rhs, const fp_t x);
int ep_G2_on_curve(const ep_t p);
void ep_G2_curve_get_gen(ep_t g);
void ep_G2_mul_gen(ep_t r, const bn_t k);
void ep_G2_rand(ep_t p);
void ep_param_set_NSS_P1536_G2(void);
#endif
#endif /* !RLC_EP_H */
